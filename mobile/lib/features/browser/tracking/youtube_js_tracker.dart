/// Hardened In-App YouTube JavaScript Tracker
/// Injected into InAppWebView to monitor HTML5 video events, detect video ID,
/// verify Google login state, and notify Flutter in real time.
class YouTubeJsTracker {
  static const String trackingScript = """
(function() {
  if (window.__VEWRA_TRACKER_INITIALIZED__) {
    // If already initialized in this window context, just trigger a state check
  ...
  function extractSearchQuery(url) {
    ...
    try {
      ...
      var q = urlObj.searchParams.get('search_query') || urlObj.searchParams.get('q');
  ...
  function checkUrlChange() {
    var query = extractSearchQuery(window.location.href);
    ...
    if (detectedId !== currentVideoId) {
      ...
      notifyFlutter({
        ...
        playbackRate: videoElement && videoElement.playbackRate ? videoElement.playbackRate : 1.0,
  ...
  function onPlay() {
    ...
    notifyFlutter({
      ...
      playbackRate: videoElement ? videoElement.playbackRate : 1.0,
      isPlaying: true
    });
  }
